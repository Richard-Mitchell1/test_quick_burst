...
def run_qb(N_slow, T_max, n_chain, pulsars, max_n_wavelet=1, min_n_wavelet=0, n_wavelet_prior='flat', n_wavelet_start='random', RJ_weight=2, glitch_RJ_weight=2,
            regular_weight=2, noise_jump_weight=2, PT_swap_weight=2, DE_prob = 0.6, fisher_prob = 0.3, prior_draw_prob = 0.1, thin_de = 10, T_ladder=None, T_dynamic=False, T_dynamic_nu=300, T_dynamic_t0=1000, PT_hist_length=100,
    ...
    pta, QB_FP, QB_FPI, glitch_indx, wavelet_indx, per_puls_indx, per_puls_rn_indx, per_puls_wn_indx, rn_indx, num_per_puls_param_list = get_pta(pulsars, vary_white_noise=vary_white_noise, include_equad=include_equad,
    ...
    all_noiseparam_idxs = [j for list in per_puls_indx for j in list]
    print('all noise param indexes: {}'.format(all_noiseparam_idxs))
    if n_chain < 3:
        print('Not enough chains for DE jumps. Make sure to set n_chain to 3 or more chains. Setting DE_prob = 0.')
        DE_prob = 0
...
Tau-scan-proposals: {1:.2f}%\nGlitch tau-scan-proposals: {5:.2f}%\nJumps along Fisher eigendirections: {2:.2f}%\nNoise jump: {4:.2f}%".format(swap_probability*100,
    ...
    if DE_prob > 0:
        de_history = initialize_de_history(n_chain, samples, QB_FPI, num_params, de_history_size = 5000, n_fast_to_slow = n_fast_to_slow, thin_de = thin_de)
    ...
    for i in range(int(start_iter), int(stop_iter)): #-1 because ith step here produces (i+1)th sample based on ith sample
        ...
        if i%n_fast_to_slow==0:
            ...
            #jump to change some noise parameters
                ...
                test_noise_jump(n_chain, max_n_wavelet, max_n_glitch, pta, QB_FPI, QB_logl, QB_Info,
            ...
            if DE_prob > 0:
                de_history = update_de_history(n_chain, samples, de_history, QB_FPI, num_params, de_history_size = 5000, sample_idx = i, n_fast_to_slow = n_fast_to_slow, save_every_n = save_every_n, thin_de = n_fast_to_slow)
...
def noise_jump(n_chain, max_n_wavelet, max_n_glitch, pta, FPI, QB_logl, QB_Info,
                ...
                num_noise_params, vary_white_noise, vary_rn, log_likelihood, wavelet_indx,
                glitch_indx, N_Noise_Params_changed):
    ...
    for j in range(n_chain):
        ...
        #Picks pulsars equal to N_noise_Params and changes
...
def test_noise_jump(n_chain, max_n_wavelet, max_n_glitch, pta, FPI, QB_logl, QB_Info,
    ...
    # print('shape of history array: {}'.format(np.shape(de_history)))
    # print('Noise param indexes: {}'.format(all_noiseparam_idxs))
    for j in range(n_chain):
        n_wavelet = int(samples[j,i,0]) #get_n_wavelet(samples, j, i) # samples[j, i]
        n_glitch = int(samples[j,i,1]) #get_n_glitch(samples, j, i)
        ...
        if which_jump == 0:
            ...
            #sum together number of intrinsic noise parameters
            ndim = sum([len(per_puls_indx[psr]) for psr in range(len(pta.pulsars))])
            # print('samples_current', samples_current)
            ...
            new_point = np.copy(samples_current)
            # print('DE proposals: {}, {} -- subtraction: {}'.format(x1, x2, x2-x1))
            #for 10% of DE jumps, do a big DE jump
            if np.random.uniform() < 0.1:
                alpha = 1
                new_point[all_noiseparam_idxs] += alpha*(x2 - x1)
                # print('new point w/ DE jump: {}'.format(new_point))
            #otherwise, do a regular DE jump
            else:
                alpha = 2.38/np.sqrt(2*ndim)
                new_point[all_noiseparam_idxs] += alpha*(x2 - x1)
                # print('new point w/ DE jump: {}'.format(new_point))
        elif which_jump == 1 or which_jump == 2:
            #perform jump along fisher eigendirections

        # elif which_jump == 3:
        #         #perform jump from prior draws

            #do the wn jump

            #Pick parameter eigenvectors to jump along
            jump_noise = np.zeros(eig_per_psr[j,0,:].shape)
            for nnn in range(N_Noise_Params_changed):
                jump_select = np.random.randint(eig_per_psr.shape[1])
                #3/21/24 change name, as this can be rn and/or wn
        ...
        # if which_jump == 0:
        #     print('DE jump new point: {}'.format(list(zip(new_point, [QB_logl[j].pta_param_names[s] for s in all_noiseparam_idxs]))[:]))
...
def get_pta(pulsars, vary_white_noise=True, include_equad = False, include_ecorr = False, include_efac = False, wn_backend_selection=False, noisedict=None, include_rn=True, vary_rn=True, include_per_psr_rn=False, vary_per_psr_rn=False, max_n_wavelet=1, efac_start=1.0, rn_amp_prior='uniform', rn_log_amp_range=[-18,-11], rn_params=[-14.0,1.0], wavelet_amp_prior='uniform', wavelet_log_amp_range=[-18,-11], per_psr_rn_amp_prior='uniform', per_psr_rn_log_amp_range=[-18,-11], equad_range = [-8.5, -5], ecorr_range = [-8.5, -5], prior_recovery=False, max_n_glitch=1, glitch_amp_prior='uniform', glitch_log_amp_range=[-18, -11], t0_min=0.0, t0_max=10.0, f0_min=3.5e-9, f0_max=1e-7, tau_min=0.2, tau_max=5.0, TF_prior=None, use_svd_for_timing_gp=True, tref=53000*86400):
    ...
    return pta, QB_FP, QB_FPI, glitch_indx, wavelet_indx, per_puls_indx, per_puls_rn_indx, per_puls_wn_indx, rn_indx, num_per_puls_param_list
...
def initialize_de_history(n_chain, samples, FPI, num_params, de_history_size = 5000, n_fast_to_slow = 10000, thin_de = 10):
    """
    ...
    for chain in range(n_chain):
        for idx in range(n_fast_to_slow//thin_de):
...
def update_de_history(n_chain, samples, de_history, FPI, num_params, sample_idx, de_history_size = 5000, n_fast_to_slow = 10000, save_every_n = 10000, thin_de = 10000):
    """
    ...
    for j in range(n_chain):
        ...
        for itrd in range(0,n_de_update):
            ...
            de_history[j,(sample_idx*n_de_update+itrd)%de_history_size] = samples[j,itrbd,2:]

    #See where the two have changed (should only be one sample set each time)
    de_mask = old_de_history != de_history

    #print(de_history[de_mask])
    #print(np.shape(de_history[de_mask]))
    return de_history

def DE_jump():
    """
    Perform differential evolution (DE) jump
